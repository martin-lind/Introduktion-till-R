...
Fortsätt med att läsa in datasetet och ge det ett namn, i det här fallet kallar vi det `dunlin_data.` En detaljerad beskrivning i hur man läser in filer finns i vår tidigare tutorial [Läsa in data i R](https://martin-lind.github.io/Introduktion-till-R/tutorials/read-data/#l%C3%A4s-in-filen).
...
Vi får en varning `Warning: cannot compute exact p-value with ties`. Det betyder att testet upptäcker att vissa värden är identiska (vi har många nollor i vårt dataset) och att p-värderna därmed inte går att lita på fullt ut. 

Vi kan lösa det problemet genom att göra analysen i funktionen `wilcox_test()` (jag vet, snarlikt namn) i paketet `coin`. Om du inte sedan tidigare har paketet installerat så gör du det med koden `install.packages("coin")`. Innan du använder paketet behöver du läsa in det i din session i R genom funktionen `library()`

```{r}
#| code-fold: false
library(coin)

m.dunlin.2 <- wilcox_test(offspring ~ as.factor(treatment),
                    data = dunlin_data)
m.dunlin.2
```
...
Vi kan nu inspektera resultatet. Vi får teststatistika (W-värde i första analysen, Z-värde i andra analysen) samt ett p-värde. Vi får inga frihetsgrader i det här testet.

Eftersom vi fick en varning när vi körde den första analysen så väljer vi att presentera resultatet från vår andra analys.
...
Ladda ner följande fil [spearman.txt](data/spearman.txt) (högerklicka, välj "spara länk som") och spara filen på din hårddisk i en mapp med ett lämpligt namn.
