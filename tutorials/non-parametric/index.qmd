...
### Statistisk modellering
Vi gör nu en icke-parametrisk korrelation, och den har stora likheter med correlationen vi tidigare gjort. Vi använder oss fortfarande av funktionen `cor.test()`, den enda skillnaden är att vi anger `method = "s"` (står för Spearman) där vi tidigare angett `"p"`
```{r}
...
### Skillnad i resultat mot parametrisk korelation
Vad hade hänt om vi analyserat datat med en parametrisk korrelation? Vi testar!

```{r}
m.pearson <-cor.test(spearman_data$A, spearman_data$B, method = "p")
m.pearson
```
Vi får nu ett signmifikant p-värde, och om vi inte hade vetat att våra data inte var normalfördelade hade vi dragit den felaktiga slutsatsen att A och B var positivt korrelerade. Men eftersom datat inte uppfyller kraven för att få göra en parametrisk korrelation så kan vi inte lite på o-värderna från en sådan korrelation.

## Test för normalfördelning

Jag rekommenderar inte att du testar för normalfördelning om du har stora dataset. Testen undersöker avvikelse från normalfördelning, och ju mer data du har, desto kraftfullare blir testen och kan upptäcka minimala avvikelser som inte spelar någon praktisk roll för de statistiska testen. Men om du nödvändigtvis vill ha ett test så använd Shapiro Wilk's test:
